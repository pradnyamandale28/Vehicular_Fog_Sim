"""
PCM Load Sweep Runner
Runs PCM scheme across load range with multiple repetitions
"""
...
import os
import sys
from Schemes.PCM_v1 import start_sumo, run_pcm
...
CSV_COLUMNS = [
    ...
    "predeployment_count", "handoff_with_replica", "handoff_without_replica",
...
def main():
    parser = argparse.ArgumentParser(description="PCM load sweep experiment")
    ...
    parser.add_argument("--output", type=str, default="results/pcm_logs_new.csv",
    ...
    print("=" * 60)
    print("PCM Load Sweep Experiment")
    print("=" * 60)
    ...
    for load in loads:
        for rep in range(args.runs):
            ...
            try:
                ...
                metrics = run_pcm(load=load, run_id=run_idx, seed=seed, gui=args.gui)
                ...
                print(f"  Pre-deployments: {metrics['predeployment_count']}, "
                      f"Fast handoffs: {metrics['handoff_with_replica']}, "
                      f"Slow handoffs: {metrics['handoff_without_replica']}")
    ...
    print("\n" + "=" * 60)
    print("PCM Experiment Completed!")
