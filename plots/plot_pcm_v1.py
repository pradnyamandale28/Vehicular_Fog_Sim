"""
PCM Individual Plots
Generates 6 plots for PCM scheme only:
...
FIGURE_HEIGHT = 4.5
DPI = 300
OUTPUT_DIR = 'results/figures/pcm'
...
def plot_dropped(df):
    fig, ax = setup_ax()
    draw_line(ax, df['load'], df['dropped'], 'PCM')
    save_fig(ax,
             xlabel='Number of Incoming Requests',
             ylabel='Dropped Requests',
             title='PCM – Dropped Requests vs Load',
             filename='pcm_dropped_vs_load.png')


def plot_accepted(df):
    fig, ax = setup_ax()
    accepted = df['total_arrivals'] - df['blocked']
    draw_line(ax, df['load'], accepted, 'PCM')
    save_fig(ax,
             xlabel='Number of Incoming Requests',
             ylabel='Accepted Requests',
             title='PCM – Accepted Requests vs Load',
             filename='pcm_accepted_vs_load.png')


def plot_success(df):
    fig, ax = setup_ax()
    draw_line(ax, df['load'], df['success'], 'PCM')
    save_fig(ax,
             xlabel='Number of Incoming Requests',
             ylabel='Successful Requests',
             title='PCM – Successful Requests vs Load',
             filename='pcm_success_vs_load.png')
...
def plot_delay(df):
    if 'avg_delay_steps' not in df.columns:
        ...
        return
    fig, ax = setup_ax()
    # SLOT_LEN_S = 0.02 s = 20 ms  (from PCM_v1.py)
    delay_ms = df['avg_delay_steps'] * 20.0
    draw_line(ax, df['load'], delay_ms, 'PCM')
    save_fig(ax,
             xlabel='Number of Incoming Requests',
             ylabel='Average End-to-End Delay (ms)',
             title='PCM – Average Delay vs Load',
             filename='pcm_delay_vs_load.png')


def plot_blocked(df):
    fig, ax = setup_ax()
    draw_line(ax, df['load'], df['blocked'], 'PCM')
    save_fig(ax,
             xlabel='Number of Incoming Requests',
             ylabel='Blocked Requests',
             title='PCM – Blocked Requests vs Load',
             filename='pcm_blocked_vs_load.png')


def plot_arrivals(df):
    fig, ax = setup_ax()
    draw_line(ax, df['load'], df['total_arrivals'], 'PCM')
    save_fig(ax,
             xlabel='Number of Incoming Requests',
             ylabel='Total Arrivals',
             title='PCM – Total Arrivals vs Load',
             filename='pcm_arrivals_vs_load.png')


# ==================== MAIN ====================
def main():
    print("=" * 60)
    print(" PCM – Individual Scheme Plots")
    print("=" * 60)

    # ── locate CSV ────────────────────────────────────────
    candidates = [
        'results/pcm_logs_new.csv',
        'results/pcm_logs.csv',
    ]
    csv_path = None
    for c in candidates:
        if os.path.exists(c):
    ...
    # ── summary ───────────────────────────────────────────
    print("\n" + "=" * 60)
    print(" ✓ All PCM plots generated!")
    print("=" * 60)
    print(f" Output folder : {OUTPUT_DIR}/")
    print("   pcm_dropped_vs_load.png")
    print("   pcm_accepted_vs_load.png")
    print("   pcm_success_vs_load.png")
    print("   pcm_delay_vs_load.png")
    print("   pcm_blocked_vs_load.png")
    print("   pcm_arrivals_vs_load.png")
    print("=" * 60)
